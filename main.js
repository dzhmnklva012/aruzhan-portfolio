...
const I18N = {
  en: {
    ...
    'services.eyebrow': 'What I bring', 'services.head': 'Where I\'m most useful',
    'svc1.t': 'Complex B2B products', 'svc1.d': 'Dense workflows, regulatory logic, and edge cases — the screens most designers avoid. I map the journey first, then design flows an analyst can move through under pressure.', 'svc1.tag': 'Full-time',
    'svc2.t': 'Design systems', 'svc2.d': 'Tokens, components, and the rules that hold them together — so a product stays consistent while three people keep adding to it. Light and dark from day one.', 'svc2.tag': 'Ongoing',
    'svc3.t': 'AI product design', 'svc3.d': 'Interfaces for products where the answer comes from a model — showing confidence, sources, and what to do when the system is wrong. I design with AI tools daily, not as a side interest.', 'svc3.tag': 'Selected projects',
  ...
  ru: {
    ...
    'services.eyebrow': 'Что я приношу', 'services.head': 'Где я полезнее всего',
    'svc1.t': 'Сложные B2B-продукты', 'svc1.d': 'Плотные сценарии, регуляторная логика и крайние случаи — экраны, которых большинство дизайнеров избегает. Сначала раскладываю путь пользователя, потом проектирую сценарии, по которым аналитик проходит в условиях дедлайна.', 'svc1.tag': 'В штат',
    'svc2.t': 'Дизайн-системы', 'svc2.d': 'Токены, компоненты и правила, которые их держат, — чтобы продукт оставался консистентным, пока в него добавляют трое. Светлая и тёмная тема с первого дня.', 'svc2.tag': 'Постоянно',
    'svc3.t': 'AI-дизайн продуктов', 'svc3.d': 'Интерфейсы для продуктов, где ответ даёт модель, — как показать уверенность, источники и что делать, когда система ошиблась. Я работаю с AI-инструментами каждый день, а не из любопытства.', 'svc3.tag': 'Избранные проекты',
